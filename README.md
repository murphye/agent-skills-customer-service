...
A non-trivial customer service agent implemented as a [Claude Code Agent Skill](https://docs.anthropic.com/en/docs/claude-code), powered by two MCP servers. It demonstrates that Agent Skills can handle complex, multi-step agentic workflows with real business logic, and that those workflows can be tested with automated, multi-turn conversation tests.

## What This Demo Shows

1. **Complex agentic logic in a skill** — a deterministic 10-step loop with state management, policy-driven decisions, and confidence-based routing.
